...
export function setupSocketIO(io: any) {
  ...
  io.on('connection', (socket: any) => {
    ...
    socket.on('webrtc-call-user', (data: { targetUserId: string; callerId: string; callerName: string; offer?: any; rideId?: string }) => {
    ...
    socket.on('webrtc-call-ride', async (data: { rideId: string; callerId: string; callerName: string; targetUserId?: string; offer?: any }) => {
      ...
      try {
        ...
        if (rideRes.rows.length > 0) {
          ...
          const recipientId = (data.callerId === r.rider_id) ? r.driver_user_id : r.rider_id;
          if (recipientId && recipientId !== data.targetUserId) {
            io.to(recipientId).emit('webrtc-incoming-call', payload);
          }
        }
      } catch (err) {
        console.warn('Lookup webrtc-call-ride:', err);
      }
