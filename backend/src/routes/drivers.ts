...
// ─── POST /api/drivers/auth-ticket (Réservé exclusivement au compte chauffeur de test) ──
router.post('/auth-ticket', async (req: Request, res: Response) => {
  try {
    ...
    // Tout autre compte chauffeur doit passer par l'authentification standard sécurisée Clerk
    return res.status(401).json({
      success: false,
      error: 'Ticket non autorisé. Les chauffeurs de production utilisent le flux standard sécurisé Clerk.',
    });
  } catch (err: any) {
    console.error('Erreur auth ticket chauffeur test:', err);
